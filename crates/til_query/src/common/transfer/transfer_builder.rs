...
impl<E: TryResult<Element>> TryFrom<Vec<E>> for TransferBuilder {
    type Error = Error;
...
impl<const SIZE: usize, E: TryResult<Element>> TryFrom<[E; SIZE]> for TransferBuilder {
    type Error = Error;
...
impl<'a, I, E> TryFrom<(I, &'a str)> for TransferBuilder
where
...
mod tests {
    ...
    fn try_result_compare() -> Result<()> {
        let transfer_1: TransferBuilder = TransferBuilder::new(
            [
        ...
        let transfer_3: TransferBuilder = TransferBuilder::new(
            [
                Element::new_data([true, false, true, true]),
                Element::new_data([true, false, false, false]),
            ],
